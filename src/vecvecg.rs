...
where
    T: Sync + Clone + PartialOrd + Into<f64>,
    ...
    Vec<U>: IntoParallelIterator,
{
    ...
    fn rightmultv(self, v: &[U]) -> Result<Vec<f64>, RE> {
        ...
        Ok((0..self[0].len())
            .map(|colnum| v.columnp(colnum, self))
            .collect())
    }
    ...
    fn matmult(self, m: &[Vec<U>]) -> Result<Vec<Vec<f64>>, RE> {
        ...
        Ok(self
            ...
            .map(|srow| {
                (0..m[0].len())
                    .map(|colnum| srow.columnp(colnum, m))
                    .collect()
            })
            .collect::<Vec<Vec<f64>>>())
    }
    ...
    fn trend(self, eps: f64, v: Vec<Vec<U>>) -> Result<Vec<f64>, RE> {
        if self[0].len() != v[0].len() {
            return data_error("trend dimensions mismatch");
        };
    ...
    fn wsigvec(self, idx: &[usize], ws: &[U]) -> Result<Vec<f64>, RE> {
        let dims = self[0].len();
        if self.len() != ws.len() {
            return data_error("wsigvec: weights number mismatch");
        };
    ...
    fn wdepth(self, descending_index: &[usize], ws: &[U], p: &[f64]) -> Result<f64, RE> {
        ...
        for &i in descending_index {
            ...
            if s.dotp(p) > p2 {
                sumvec.mutvadd(&s.smult(ws[i].clone().into() / ssq.sqrt()))
            };
        }
    ...
    fn dependencies(self, m: &[U]) -> Result<Vec<f64>, RE> {
        ...
        return self
            .par_iter()
            .map(|s| -> Result<f64, RE> { Ok((entropym + s.entropy()) / s.jointentropy(m)? - 1.0) })
            .collect();
    }

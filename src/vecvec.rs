...
use crate::{Med, MStats, MinMax, MutVecg, MutVecf64, Stats, VecVec, VecVecg, Vecg};
...
impl<T,U> VecVecg<T,U> for &[Vec<T>] where T: Copy+PartialOrd, 
    f64: From<T>, T: From<f64>, U: From<f64>,
    U: Copy+PartialOrd,f64: From<U>  {
    ...
    fn medoid(self) -> MinMax<f64> {  
        let (_,res) = self.distsums(); 
        // let dists:Vec<f64> = ds.iter().map(|&d| d).collect();
        res
    }
   */
    ...
    fn eccinfo(self, eps: f64) -> (MStats, Med, MinMax<U>) where Vec<U>:FromIterator<f64> {
        let gm:Vec<f64> = self.gmedian(eps);
        let eccs:Vec<U> = self.iter().map(|v| gm.vdist(&v)).collect();
